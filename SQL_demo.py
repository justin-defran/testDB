...
import numpy as np
from .decorators import decorator_insert, decorator_extract
...
class SQL_demo(SQL_abstract):
    ...
    def __init__(self):
        ...
        self.read_bin_dir = '..\\demo-app\\figs\\'
        self.write_bin_dir = '..\\demo-app\\'

    @decorator_insert
    def insert(self, filename, param, x, y):

        query = 'INSERT INTO ' + self.TABLE_NAME + '(data_id, bin, filename, param, x, y, time_stamp) ' \
                'VALUES (%s,%s,%s,%s,%s,%s,CURRENT_TIMESTAMP);'

        content = (self.data_id, get_bin(self.read_bin_dir + filename), filename, param, psycopg2.Binary(x.tobytes()), psycopg2.Binary(y.tobytes()))
        return (query,content)

    @decorator_extract
    def extract(self, data_id=None, key=None, filename=None, param=None):
        ...
        if (data_id != None):
            ...
            name = None
        content = []
        input = [name, key, filename, param]

        input_strings = [" data_id LIKE %s", " and key = %s", " and filename = %s", " and param = %s"]
    ...
    def format(self, raw):
        ...
        for row in raw:
            f = open(self.write_bin_dir + row[3], 'wb')
            f.write(row[2])
            f.close()

        data.x = [np.frombuffer(row[5], dtype=np.int32) for row in raw]
        data.y = [np.frombuffer(row[6], dtype=np.float64) for row in raw]
